...
# INVENTORY SYSTEM (Array-only)

class InventoryArraySystem:
    def __init__(self):
        self.array_storage = []  # array storage only
    ...
    def insert_product(self):
        ...
        self.array_storage.append(prod)

        print("\nProduct inserted successfully!")

    def edit_product(self):
        pid = input("Enter Product ID to edit: ")
        product = self.search_array(pid)
        if product:
    ...
    def delete_product(self):
        ...
        original_len = len(self.array_storage)
        self.array_storage = [p for p in self.array_storage if p.product_id != pid]
        if len(self.array_storage) < original_len:
    ...
    def search_product(self):
        ...
        if product_arr:
            print("\nProduct found:", product_arr)
        else:
            print("\nProduct not found in Array.")

        print("\n--- Performance ---")
        print(f"Array Search Time: {(end_arr - start_arr):.9f} ms")
    ...
    def compare_all_searches(self):
        ...
        print(f"\n--- Array Search Performance for All {num_products} Products ---\n")
        print(f"Total Array Search Time: {total_arr_time:.9f} ms\n")
        print(f"Average Array Search Time: {total_arr_time / num_products:.9f} ms per product")

    def menu(self):
        choice = ''
        while choice != '7':
            print("\n===== Baby Shop Inventory System (Array Only) =====")
